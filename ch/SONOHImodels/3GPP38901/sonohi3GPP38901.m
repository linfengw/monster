classdef sonohi3GPP38901 < sonohiBase
	...
	methods
		...
		function setup(obj, Stations, ~, ~)
			% Setup a structure of spatial maps for each station.
			for stationIdx = 1:length(Stations)
				...
				[LOSpropmap, axisLOSprop] = obj.generateLOSMap(station);
		...
		function [mapLOSprop, axisLOSprop] = generateLOSMap(obj, station)
			...
			parameters = obj.getParameters(areaType);
			% Configure LOS probability map G, with correlation distance
			% according to 7.6-18.
			[mapLOSprop, xaxis, yaxis] = obj.LOSpropMap(parameters.dCorrLOSprop, fMHz, radius,  station.Seed);
		...
		function [mapLOS, mapNLOS, axisLOS, axisNLOS] = generateShadowMap(obj, station)
			...
			[mapLOS, xaxis, yaxis] = obj.spatialCorrMap(parameters.sigmaSFLOS, parameters.dCorrLOS, fMHz, radius, station.Seed);
			axisLOS = [xaxis; yaxis];
			
			% Spatial correlation map of NLOS Large-scale SF
			[mapNLOS, xaxis, yaxis] = obj.spatialCorrMap(parameters.sigmaSFNLOS, parameters.dCorrNLOS, fMHz, radius, station.Seed);
	...
	methods(Static)
		...
		function parameters = getParameters(areaType)
			switch areaType
				case 'RMa'
					sigmaSFLOS = 5;
					sigmaSFNLOS = 8;
			...
			parameters = struct();
			parameters.sigmaSFLOS = sigmaSFLOS;
			parameters.sigmaSFNLOS = sigmaSFNLOS;
			parameters.dCorrLOS = dCorrLOS;
			parameters.dCorrNLOS = dCorrNLOS;
			parameters.dCorrLOSprop = dCorrLOSprop;
		end
