...
function plotSNR(Param, StationsIn)
    ...
    if Param.posScheme == "Single Cell" && Param.seed == 42 && isfile('utils/plotlib/preplottedResults/SNRSingleCell42.mat')
       load('utils/plotlib/preplottedResults/SNRSingleCell42.mat');
        ...
        UMaResultsSNR = nan(N,N,length(Stations));
        ...
        for iStation=1:length(Stations)
            ...
            for Xpos = 1:length(lengthXY(1,:))
                ...
                for Ypos = 1:length(lengthXY(2,:))
                    ...
                    try
                        ...
                        resultsUMa{iStation,Xpos,Ypos}.SNRdB = ueUMaS.Rx.SNRdB;
                    counter = counter +1;
            ...
            for Xpos = 1:length(lengthXY(1,:))
                ...
                for Ypos = 1:length(lengthXY(2,:))
                    ...
                    UMaResultsSNR(Xpos,Ypos,iStation) = resultsUMa{iStation,Xpos,Ypos}.SNRdB;
        ...
        %Save to reduce setup time
        if Param.posScheme == "Single Cell" && Param.seed == 42
            save('utils/plotlib/preplottedResults/SNRSingleCell42.mat', 'UMaResultsSNR');
        end
    end
    ...
    %plot for macro
    if Param.numMacro > 0
        contourf(Param.LayoutAxes(7),lengthXY(1,:), lengthXY(2,:), max(UMaResultsSNR(:,:,1:Param.numMacro),[],3), 10)
        c = colorbar(Param.LayoutAxes(7));
        c.Label.String = 'Max SNR [dB]';
        c.Label.FontSize = 12;
        colormap(hot)
    end
    ...
    %plot for micro
    if Param.numMicro > 0
        contourf(Param.LayoutAxes(8),lengthXY(1,:), lengthXY(2,:), max(UMaResultsSNR(:,:,Param.numMacro+1:Param.numMacro+Param.numMicro),[],3), 10)
    ...
    if Param.numPico > 0
        contourf(Param.LayoutAxes(9),lengthXY(1,:), lengthXY(2,:),... 
                max(UMaResultsSNR(:,:,Param.numMacro+Param.numMicro+1:Param.numMacro+Param.numMicro+Param.numPico),[],3), 10)
